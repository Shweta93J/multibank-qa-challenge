...
test.describe('Authentication', () => {
 ...
 test('login form is rendered correctly @regression', async ({ page }) => {
  const loginPage = new LoginPage(page);
  await loginPage.open();
  await expect(page.locator(selectors.auth.emailInput.join(', ')).first()).toBeVisible();
  await expect(page.locator(selectors.auth.passwordInput.join(', ')).first()).toBeVisible();
  await expect(page.locator(selectors.auth.loginButton.join(', ')).first()).toBeVisible();
});
